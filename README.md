...
contract CarInsurance {
    ...
    uint256 constant public premium = 0.1 ether;
    uint256 constant public coverage = 1 ether;
    uint256 public expiration;
    address payable public owner;
    address public car;
    
    constructor(address _car) payable {
        require(msg.value == premium, "Please pay the correct premium.");
        owner = payable(msg.sender);
        car = _car;
        expiration = block.timestamp + 1 years;
    }
    
    function renew() external payable {
        require(msg.sender == owner, "Only the owner can renew the insurance.");
        require(msg.value == premium, "Please pay the correct premium.");
        require(block.timestamp > expiration, "The insurance is not expired yet.");
        expiration = expiration + 1 years;
    }
    
    function makeClaim() external {
        require(msg.sender == car, "Only the car can make a claim.");
        require(block.timestamp < expiration, "The insurance has expired.");
        payable(car).transfer(coverage);
